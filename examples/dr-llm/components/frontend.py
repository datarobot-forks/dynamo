...
class Frontend:
    ...
    def setup_model(self):
        ...
        logger.info(f"Setting up model using dynamo namespace: {self.frontend_config.namespace}")
        subprocess.run(
            [
                "llmctl",
                "-n",
                self.frontend_config.namespace,
                "http",
                "remove",
                "chat-models",
    ...
    def start_http_server(self):
        ...
        logger.info(f"HTTP server using dynamo namespace: {self.frontend_config.namespace}")
        self.process = subprocess.Popen(
            [http_binary, "-p", str(self.frontend_config.port), "--namespace", self.frontend_config.namespace],
            stdout=None,
            stderr=None,
        )
    ...
    def cleanup(self):
        ...
        logger.info(f"Cleaning up model using dynamo namespace: {self.frontend_config.namespace}")
        subprocess.run(
            [
                "llmctl",
                "-n",
