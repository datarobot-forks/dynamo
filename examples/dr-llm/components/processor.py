...
class Processor(ProcessMixIn):
    ...
    async def async_init(self):
        ...
        logger.info(f"DATAROBOT_DEBUG dynamo_context runtime: {runtime}")
        comp_ns, comp_name = VllmWorker.dynamo_address()  # type: ignore
        logger.info(f"DATAROBOT_DEBUG vLLMWorker namespace: {comp_ns} component_name: {comp_name}")
        self.worker_client = (
        ...
        if self.use_router:
            ...
            logger.info(f"DATAROBOT_DEBUG Router namespace: {router_ns} component_name: {router_name}")
            self.router_client = (
        ...
        kv_listener = runtime.namespace(comp_ns).component("VllmWorker")
